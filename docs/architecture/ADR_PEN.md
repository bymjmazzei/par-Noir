...
6. **Browse `contentClass`** remains the publish target only. `docType` is compile/preview hint. Client may pin categories, search the catalog, and toggle home **All** / **By category** / **Dashboard**; prefs are local. Dashboard defaults to Calendar, Schedule, To-do, Recent notes widgets on a shared drag layout surface.
...
8. **Rich text SoT:** Section body is TipTap/ProseMirror JSON (`PenSectionContent.doc`). Optional `PenSectionContent.layers` (text/image page objects with % rects). Dual-pane editor: left flow TipTap edits the active text layer; right editable page (`LayoutSurface`) places layers. Legacy `{ blocks }` migrates once on load. Compile emits plain `content` + filled `style` (+ optional `doc`) for browse. In-doc `pen.comment` / `pen.suggestion` ride the Pen outbox → apply-inbound; public comments stay on `/api/engagement/:fileId/*`. Manifest `pageLayout: flow|letter|a4`. No multi-cursor CRDT.
