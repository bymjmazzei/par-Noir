...
7d. **Verified gates (fail closed):** `isVerifiedAuthor` is `false` until Veriff. Unverified: private Yours / cloud templates + Use public templates. Verified: share to public aggregators (browse / pen-templates) and API-key private for self-hosted community. UI exposes disabled public share with copy when unverified.
