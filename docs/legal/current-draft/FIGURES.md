...
- pN client 102 reads/writes identity file 104 locally.
- Long-lived cloud credentials stay on device 100 (sealed cloud credentials 106); client 102 forwards an ephemeral cloud access token to API 120 for owner storage I/O; API 120 does **not** retain long-lived Drive OAuth secrets.
- API 120 holds OAuth client registry 122, not user unlock secrets.
- Membership alignment module 124 keeps aggregator cache 130 consistent with public index 112 (see FIG. 10).
...
flowchart LR
  subgraph userSide [UserSide]
    ...
    sealedCreds[Sealed_cloud_creds]
  ...
  client102 --> pnFile104
  client102 --> sealedCreds
  sealedCreds -->|"forwarded_AT"| api120
  client102 --> api120
  api120 --> cloud110
  api120 --> cache130
  browser140 --> api120
...
par Noir - {pnIdentifier}/
  ...
  par-noir-messages/           (on demand)
    _outbox/                   (sender durable commit under custody)
    conversation-*.xlsx
```

**Note:** Peer private delivery materializes on the **recipient** device into the recipient's own `par-noir-messages/`; senders do not write cross-user into another user's Drive with peer credentials.

---

## FIG. 10 — Public index membership alignment

**Type:** Flowchart

**Description:**

1. Owner publish pushes catalog + publicContentRef into cache 130 (step 1000a).
2. Explicit delete updates public index 112 and immediately purges cache 130 (step 1000b / claim 15).
3. Public-content fetch 404/410 → purge dead cache row (step 1000c).
4. Owner-device reconcile with forwarded cloud access token: for each of owner's public cache rows, compare to index 112 / blob presence (steps 1002–1004); purge missing (1006–1008); on auth error skip (1010).
5. Aggregator browser 140 reads cache 130 via API 120 (1012).

Index 112 remains membership truth. The coordinating server does not retain long-lived cloud OAuth secrets.

```mermaid
flowchart TD
  start[Membership_alignment]
  start --> publish[Owner_publish]
  start --> del[Explicit_delete_purge]
  start --> blind[Public_content_404_purge]
  start --> ownRec[Owner_device_reconcile_forwarded_AT]
  publish --> cache[Cache_130]
  del --> cache
  blind --> cache
  ownRec --> cache
```
...
| 800–806 | Succession |
| 1000–1012 | Membership alignment steps |
| 1200 | ZK v2 envelope |
