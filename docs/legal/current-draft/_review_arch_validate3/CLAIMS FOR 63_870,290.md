...
7. The method of claim 6, wherein re-wrapping encrypted digital assets comprises re-wrapping encrypted data files, direct messages, group keys, and credential tokens.
...
a reconcile module executing on a background interval;
wherein the reconcile module periodically authenticates into the user-controlled cloud storage instance using credential tokens, retrieves the user-authored public index structure, and purges entries from the non-authoritative performance cache whose unique identifiers are absent from the retrieved public index structure, maintaining the user-controlled cloud storage instance as sole authoritative protocol state.
...
14. The system of claim 12, wherein the reconcile module executes a total purge of all cached entries associated with the user from the non-authoritative performance cache upon detecting that the public index structure has been completely removed from the user-controlled cloud storage instance.

15. The system of claim 12, wherein an explicit user deletion command dispatched through an application API updates the public index structure and triggers an immediate purge call to the non-authoritative performance cache without awaiting the background interval.
