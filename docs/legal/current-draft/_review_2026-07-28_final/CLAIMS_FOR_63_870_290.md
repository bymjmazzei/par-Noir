...
5. The system of claim 1, wherein the set of passcode-sealed cryptographic shares embedded within the portable identity file are encrypted via AES-GCM using a key derived from a user name string and the user passcode factor via PBKDF2 with SHA-512 and at least one million iterations.
