...
| **Paid feed subscriptions** (subscriber pays for access to a **feed**) | **Not** a par Noir–hosted product. Creators use **third-party** billing for subscriber access; see [`FEEDS_AND_THIRD_PARTY_MONETIZATION.md`](./FEEDS_AND_THIRD_PARTY_MONETIZATION.md). **Not** in **`G`**; **do not merge** into creator fund ledger lines. |
| **Creator private subscriptions, paywalls, “VIP” tiers, coaching, etc.** | **Out of par Noir billing for v1.** Creators use **feeds / public index** plus their **own third-party** tool for access control, subscriber lists, and fees—or a **future optional add-on / L5 partner**. Revenue from those tools **does not** enter the creator fund **`G`**. |
...
| **Payments rail (creator fund)** | **Stripe only** for **monetization maintenance** (money **in** via card/bank payers) and for **all** creator-fund **payouts** (money **out** via **Stripe Connect**). Other non–creator-fund surfaces (e.g. one-time Coinbase checkouts where still used) stay **out of this `G`** per [Scope](#scope-creator-fund-vs-other-paid-surfaces). |
...
**Product surface:** The **licensing portal** ([`apps/licensing-portal`](../../apps/licensing-portal)) combines **mailto intake** for new partnerships with **pN sign-in** and an authenticated **track library** backed by the API registry (phases **A–B** of [Track registry and licensing portal (build plan)](#track-registry-and-licensing-portal-build-plan)). **No** second payout vendor; **Stripe Connect** only.
