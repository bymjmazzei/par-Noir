/**
 * Capacitor Keychain session vault for Prism.
 */
...
export async function isPrismSessionVaultAvailable(): Promise<boolean> {
  const v = await getVault();
  return v.isAvailable();
}

export async function hasPrismSessionVault(): Promise<boolean> {
  const v = await getVault();
  return v.hasVault(APP_ID);
}

export async function enrollPrismSessionVault(sessionJson: string): Promise<void> {
  const v = await getVault();
  const payload: PrismSessionPayload = { kind: 'prism_session', sessionJson };
  await v.enrollAfterUnlock(APP_ID, payload);
}

export async function unlockPrismSessionVault(): Promise<SessionVaultPayload | null> {
  const v = await getVault();
  return v.unlockWithBiometric(APP_ID);
}

export async function clearPrismSessionVault(): Promise<void> {
  const v = await getVault();
