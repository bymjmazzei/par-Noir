...
export class CoinbaseProxy {
  ...
  static async createCheckoutViaProxy(checkoutData: CheckoutRequest): Promise<CoinbaseCheckout> {
    ...
    try {
      ...
      const COINBASE_API_KEY = integrationsEnv.COINBASE_COMMERCE_API_KEY ||
                               IntegrationConfigManager.getApiKey('coinbase', 'COINBASE_COMMERCE_API_KEY');

      if (!COINBASE_API_KEY) {
        throw new Error('Coinbase Commerce API key not configured. Set VITE_COINBASE_COMMERCE_API_KEY or configure in Integration Settings.');
      }
