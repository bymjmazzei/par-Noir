...
- **par Noir as infrastructure.** Identity = layer 1; dashboard = layer 2; API = layer 3; browser = layer 4; third parties = layer 5. User data and content are portable across pN-based systems. Goal: invert surveillance capitalism—users aggregate and broker their own data; systems stay feature- and value-focused instead of exploitative. For messaging: E2E content is client-encrypted; the API coordinates delivery and must minimize operator retention of routing metadata—see `docs/security/MESSAGING_COORDINATOR_POLICY.md`.
