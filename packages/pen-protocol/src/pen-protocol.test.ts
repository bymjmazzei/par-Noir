...
describe('template seeds + Mini featured + layer locks', () => {
  ...
  it('consumer starters ship format-first seeds with presentation', () => {
    ...
    expect(() => requireTemplate('journal.basic.v1')).toThrow(/unknown_pen_template/);
    expect(() => requireTemplate('list.basic.v1')).toThrow(/unknown_pen_template/);
    expect(() => requireTemplate('landing.basic.v1')).toThrow(/unknown_pen_template/);
  ...
  it('listLayerParts includes Body plus headings and blockquotes', async () => {
    const { listLayerParts, insertLayerPart, partIdAtContentIndex, reorderLayerParts } =
      await import('./index.js');
    const empty = listLayerParts(undefined);
    expect(empty[0]?.kind).toBe('body');
    ...
    const reordered = reorderLayerParts(doc, heading.index, parts.length - 1);
    const after = listLayerParts(reordered);
